...
creating_div.innerHTML=`
  <p for="id">id :${obtained_data.id}</p>
...
btn_next.addEventListener('click',()=>{
    var user_data=100-5
    var iter_value=parseInt(user_data/5)*5
    for(i=1;i<=5;i++){
    var taking_for=document.querySelector(`.btn-${i}`)   
    var attribute_val=parseInt(taking_for.getAttribute('for'))
    if(attribute_val<=iter_value){
    var increasing_val=attribute_val+5
    taking_for.setAttribute('for',increasing_val)
    taking_for.innerHTML=increasing_val
    }
    }
})
